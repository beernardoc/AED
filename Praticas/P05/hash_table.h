...
hash_table_node *find_hash_table_node(char key[64])
{
  ...
  // 1. CALCULAR O IDX A PARTIR DA CHAVE
  // 2. VERIFICAR SE HASHTABLE[IDX] É NULO:
  //      SE SIM - ENTAO A CHAVE NAO ESTA NA HASHTABLE -> RETURN NULL
  //      SE NAO - VERIFICAR SE HASHTABLE[IDX] == KEY:
  //          SE SIM - ENCONTREI O NO DESEJADO
  //          SE NAO - CONTINUAR A PERCORRER A LISTA ATE ESTA TERMINAR OU ATE ENCONTRAR O NO DESEJADO

  int HashIndex = hash_function(key); // 1
  n = hash_table[HashIndex]; // valor presente no hash_table(HashIndex)
...
void insert_hash_table_node(hash_table_node *n)
{
  ...
  // 1. CALCULAR O IDX A PARTIR DE N -> KEY 
  // 2. N-NEXT = HASH_TABLE[IDX]
  // 3. HASH_TABLE[IDX] = N;

  int HashIndex = hash_function(n->key);
  n->next = hash_table[HashIndex];
  hash_table[HashIndex] = n;
