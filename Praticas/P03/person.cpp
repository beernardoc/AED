...
class person
{
  ...
  public:
    person(const char *name = "",const char *address = "", int number = 0);// constructor
...
person::person(const char *name,const char *address, int number)
{ // constructor
  if(debug != 0)
...
{ // make the << operator also work with the person data type                 toString do java
  return os << "[Name]: "
            << p.get_name()
            << "  [Address]: "
            << p.get_address()
...
int main(void)
{
  person p = person("John Doe","no address", 910632432); // create a new object and then copy it to p
  person q("no name","no address");           // create a new object (no copy needed)

  cout << "Before change of address: " << p.get_name() << ", " << p.get_address() << "," << p.get_number() << endl;
